...
function activate(context) {
    ...
    function updateDecorations() {
        ...
        const levelColorsRaw = config.get('levelColors', defaultLevelColors);
        const level_colors = levelColorsRaw.slice(0, 6).map(c => Array.isArray(c) && c.length >= 3 ? c.slice(0, 3) : [150, 150, 150]);
        const themeCtx = JSON.stringify({
            accent_hl: accent,
            text_main: text_main,
            hide_labels: config.get('hideLabels', false),
            level_colors: level_colors
        });
        try {
            // Call Rust Wasm with theme context
            const spans = zq.parse_to_spans_wasm(text, themeCtx);
            // Map to decorations
    ...
    context.subscriptions.push(vscode.languages.registerDocumentSymbolProvider({ language: 'zq' }, {
        provideDocumentSymbols(document, token) {
            ...
            try {
                const labels = zq.parse_semantic_labels_wasm(text);
                // Build tree
                const rootSymbols = [];
                const stack = [];
    ...
    function lineByLineHeadings(text) {
        const syms = [];
        ...
        lines.forEach((line, i) => {
            ...
            if (headingMatch) {
                ...
                syms.push(new vscode.DocumentSymbol(name, `Level ${level}`, vscode.SymbolKind.Module, new vscode.Range(i, 0, i, line.length), new vscode.Range(i, 0, i, line.length)));
            }
        });
        return syms;
    }
    ...
    context.subscriptions.push(vscode.commands.registerCommand('zhuqian.toggle', () => {
        ...
        const config = vscode.workspace.getConfiguration('zhuqian');
        const current = config.get('hideLabels', false);
        config.update('hideLabels', !current, vscode.ConfigurationTarget.Global);
        vscode.window.setStatusBarMessage(current ? '竹签: Labels visible' : '竹签: Labels hidden', 2000);
    }));
}
class ZqSettingsPanel {
    ...
    _getHtmlForWebview() {
        const config = vscode.workspace.getConfiguration('zhuqian');
        const ff = config.get('fontFamily') || 'serif';
        const settings = {
